...
function ajouter_sous_utilisateur($db){
	if (isset($_POST['UserID']) && !empty($_POST['Mail']) )
	{
		modifier_sous_utilisateur($db);
	}
	else if (isset($_POST['UserPassword']) && isset($_POST['UserPassword2']) && isset($_POST['Mail']) && !empty($_POST['UserPassword']) && !empty($_POST['UserPassword2']) && !empty($_POST['Mail']))
	{
		if ($_POST['UserPassword'] == $_POST['UserPassword2'])
		{
			if (verification_existence_mail($db))
			{
			inscription_sous_utilisateur($db);
			}
			else
			{
				$_SESSION["erreur"] ="Ce mail existe déja";
			}
		}
		else
		{
